...
class SuperBITIsrConfig(IsrTask.ConfigClass):
...
""",
    ...
    def validate(self):
        super(SuperBITIsrConfig, self).validate()
        if self.doFlat and self.doApplyGains:
            raise ValueError("You may not specify both self.doFlat and self.doApplyGains")


class SuperBITIsrTask(IsrTask):

    ConfigClass = SuperBITIsrConfig

    def __init__(self, *args, **kwargs):
        super(SuperBITIsrTask, self).__init__(*args, **kwargs)
    ...
    def runDataRef(self, sensorRef):
        ...
        if self.config.doBias:
            ...
            sensorRef.put(ccdExposure,  "Biascorr")
